...
# features_save = '../cnn_features_model.h5'
...
class SegDataLibrary(object):
    ...
    def getDiseasedPatchs(self, radius, save_patch=False):
        ...
        for s in sentinels:
            numbers = 0
            for p in paths:
                if p != '../DataSets/Brain_Pipelined/train_save//.DS_Store':
                    name = p[p.rfind('/')+1: p.rfind('.')]
                    images_t = io.imread(p,plugin='matplotlib').reshape(5, 240, 240).astype('float')
                    # images_t = np.load(p).reshape(5, 240, 240).astype('float')
                    print(glob(label_dir + name + '*'))
                    if(len(glob(label_dir + name + '*')) == 0):
                        continue
    ...
    # 根据整张切片提取CNN特征
    def getFeaturesBySlices(self, train_save, label_save):
        self.cnn = CNNFeatures.CNNFeatures(model_dir=features_save)
        print('Starts get features...')
        # patch_scale_path = self.patch_scale_path
        excel_raw_path = self.excel_raw_path
...
def disposeRawdatas():
    ...
    for d in dcm_dir:
        if d != '../DataSets/Raw_dicom//.DS_Store':
...
def getFeaturesFromSlices():
    ...
    trainCNNFeatreus(train_path, label_path)

    seg_data_library.getFeaturesBySlices(train_save=train_path, label_save=label_path)


if __name__ == '__main__':
    # xx = np.load("D:/BraTS2019_Experiments/DataSets/Npy_Datas/BraTS20_Training_001_flair.nii_53.npy")[-1]
    # plt.imshow(xx)
    # plt.show()
    print(123)
    # seg = SegDataLibrary()
    ...
    # disposeRawdatas()
    getFeaturesFromSlices()
    # xx = np.load('D:/BraTS2019_Experiments/DataSets/seg_model/patchs/xgb_patch_train_10.npy')
    # yy = np.load('D:/BraTS2019_Experiments/DataSets/seg_model/patchs/xgb_patch_label_10.npy')
    # print(yy)
    #####检测图形形状###
